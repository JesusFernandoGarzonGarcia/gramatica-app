...
const DerivationTree = ({ grammar }) => {
  ...
  const generateDerivationTrees = () => {
    const generalTree = buildGeneralDerivationTree(grammar, inputString);
    
    // Solo genera el camino particular si el árbol general es válido.
    if (generalTree && isValid) {
      const path = buildParticularDerivationPath(generalTree, inputString);
      setParticularPath(path);
    } else {
      setParticularPath([]); // Resetea el camino particular si no es válido
    }
  };
